...
1. **Universal Precision (carried forward).** All data tensors — targets and covariates — are `float32` before entering any model or loss. On 0.2.x this holds by construction: `ViewsDataset` stores `float32`, and `ReproducibilityGate.Data.audit_frame_schema` treats a `float64` frame as informational because the converters never produce one.
...
- Clipping default: `views_r2darts2/dataset/base.py`, `ingest_darts_predictions` and `ingest_numpy_predictions`, parameter `clip_negatives`.
- No code change is required by this ADR. The change required by D-05's eventual ruling will be a one-line default flip or a DNA-gene addition, plus the corresponding update to `DartsForecaster`'s docstring and `docs/CICs/darts_forecaster.md`.
...
- D-05: keep, remove, or gene-ify `clip_negatives`.
- Whether the informational-only `float64` check in `audit_frame_schema` should become fail-loud now that the converters guarantee `float32`.
