...
- **Structural NaN → 0:** on the Darts path, `to_darts_timeseries` applies `np.nan_to_num(nan=0.0)` (`base.py:1664`) on the premise that NaN means "entity absent for those time steps". A genuinely missing observation becomes a zero silently. This is register **D-07** against the fortress protocol's explicit `nan_to_num` prohibition.
