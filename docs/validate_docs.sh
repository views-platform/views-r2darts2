...
# 7b. Artifacts that other passes silently assume exist.
echo "--- Checking register presence and loss-card index ---"
if grep -rq 'technical_risk_register\.md' --include='*.md' --exclude-dir=archive . 2>/dev/null \
   && [ ! -f "$REPO_ROOT/reports/technical_risk_register.md" ]; then
    echo "  ERROR: docs reference reports/technical_risk_register.md but it does not exist"
    errors=$((errors + 1))
fi
if [ -f loss_cards/README.md ] && [ ! -s loss_cards/README.md ]; then
    echo "  ERROR: loss_cards/README.md exists but is empty (0 bytes) — the loss-card index is missing"
    errors=$((errors + 1))
fi
