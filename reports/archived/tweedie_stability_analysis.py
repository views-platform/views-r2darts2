...
def analyze_stability():
    ...
    ax1.plot(
        eta_range.detach().numpy(),
        losses_exp.detach().numpy(),
        label="Loss with exp() link",
        color="r",
        linestyle="--",
    )
    ax1.plot(
        eta_range.detach().numpy(),
        losses_softplus.detach().numpy(),
        label="Loss with softplus() link (Current)",
        color="b",
        linewidth=2,
    )
    ...
    ax2.plot(
        eta_range.detach().numpy(),
        grads_exp.detach().numpy(),
        label="Gradient with exp() link",
        color="r",
        linestyle="--",
    )
    ax2.plot(
        eta_range.detach().numpy(),
        grads_softplus.detach().numpy(),
        label="Gradient with softplus() link (Current)",
        color="b",
        linewidth=2,
    )
    ...
    plt.suptitle(
        "Proof of Numerical Stability: Tweedie Loss Link Functions", fontsize=20, y=0.95
    )
