...
def main():
    ...
    for i in range(len(train_series_log1p)):
        ...
        baseline_predictions_country_raw = baseline_country.predict(n=n_predict_baseline_country).values().flatten()
    ...
    baseline_msle = mean_squared_log_error(aggregated_baseline_y_true, aggregated_baseline_predictions_raw)
    baseline_ratio = calculate_mean_pred_true_ratio(aggregated_baseline_y_true, aggregated_baseline_predictions_raw)
    ...
    for model_name in MODEL_CONFIGS.keys():
        ...
        evaluate.main(model_name=model_name,
