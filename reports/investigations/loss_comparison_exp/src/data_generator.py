...
def generate_clustered_synthetic_data(n_samples, p_peace_to_conflict, p_conflict_to_peace, lognormal_mean, lognormal_sigma, random_seed):
    """
    ...
    for i in range(1, n_samples):
        if states[i-1] == 0:
            ...
            states[i] = rng.choice([0, 1], p=[p_conflict_to_peace, p_conflict_to_conflict])

    n_conflict_events = np.sum(states)
    
    # Only draw magnitudes for conflict events
    if n_conflict_events > 0:
        conflict_magnitudes = rng.lognormal(mean=lognormal_mean, sigma=lognormal_sigma, size=n_conflict_events)
        raw_counts[states == 1] = conflict_magnitudes
    
    return raw_counts

def generate_multiple_clustered_synthetic_data(data_config: dict, experiment_config: dict):
    """
...
def main():
    ...
    print(f"Generating {DATA_CONFIG['n_timelines']} timelines of {DATA_CONFIG['n_samples']} steps each...")
    # Generate data for multiple timelines
    full_raw_df = generate_multiple_clustered_synthetic_data(DATA_CONFIG, EXPERIMENT_CONFIG)
    ...
    print(f"Successfully saved raw data for {DATA_CONFIG['n_timelines']} timelines to {raw_path}")
    print(f"Successfully saved log1p transformed data for {DATA_CONFIG['n_timelines']} timelines to {log1p_path}")
