...
class HeuristicBaseline(BaseModel):
    ...
    def fit(self, train_ts: TimeSeries):
        ...
        print(f"Fitting baseline: calculating mean of entire training series (length {len(train_ts)}).")
    ...
    def predict(self, n: int) -> TimeSeries:
        ...
        if self._last_train_time is None or self._freq is None:
            raise RuntimeError("Baseline not fitted: train_ts.end_time() and freq are not set.")
            
        prediction_start_time = self._last_train_time + self._freq
        time_index = pd.date_range(start=prediction_start_time, periods=n, freq=self._freq)
...
class DartsNBEATS(BaseModel):
    ...
    def __init__(self, nbeats_hps, loss_fn_class, loss_fn_params,
                 trainer_config, optimizer_config, lr_scheduler_config, early_stopping_config):
        super().__init__()
        ...
        self.model = NBEATSModel(
            ...
            optimizer_kwargs={"lr": optimizer_config["lr"], "weight_decay": optimizer_config["weight_decay"]},

            # LR Scheduler parameters (NBEATSModel takes these directly)
            lr_scheduler_cls=getattr(torch.optim.lr_scheduler, lr_scheduler_config["lr_scheduler_cls"]),
            lr_scheduler_kwargs={
    ...
    def fit(self, train_ts: TimeSeries, val_ts: TimeSeries = None): # Added val_ts parameter
        self.model.fit(series=train_ts, val_series=val_ts, verbose=False) # Modified to pass val_series
    ...
    @classmethod
    def load(cls, path, nbeats_hps, loss_fn_class, loss_fn_params,
             trainer_config, optimizer_config, lr_scheduler_config, early_stopping_config):
        # We need to reconstruct the model with the same parameters
        # before loading the state.
        instance = cls(nbeats_hps, loss_fn_class, loss_fn_params,
                       trainer_config, optimizer_config, lr_scheduler_config, early_stopping_config)
