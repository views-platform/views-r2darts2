...
from src.data_generator import main as data_generator_main # Import the data generator's main function
from src.model_definitions import HeuristicBaseline # Import the updated HeuristicBaseline
from src.utils.metrics import calculate_mean_pred_true_ratio # Import the metric function
...
def main():
    ...
    log1p_path = os.path.join(data_dir, "log1p_transformed.csv") # Need log1p for N-BEATS, but raw for baseline fit.
    try:
    ...
    full_series_log1p = TimeSeries.from_group_dataframe(df=log1p_df, time_col='time', value_cols='target', group_cols='country_id', fill_missing_dates=True, freq='M')
    full_series_raw = TimeSeries.from_group_dataframe(df=raw_df, time_col='time', value_cols='target', group_cols='country_id', fill_missing_dates=True, freq='M')
    ...
    print(f"Loaded {len(train_series_log1p)} training series and {len(val_series_raw)} validation series.")
    ...
    random.seed(EXPERIMENT_CONFIG["random_seed"]) # For reproducibility of random selection
    
    selected_indices = random.sample(range(len(full_series_log1p)), min(num_timelines_to_plot, len(full_series_log1p)))
    ...
    fig, axes = plt.subplots(len(selected_indices), 1, figsize=(15, 4 * len(selected_indices)), sharex=True)
    if len(selected_indices) == 1:
        axes = [axes] # Ensure axes is iterable even for a single plot

    for idx, (train_ts_country, val_ts_country_raw) in enumerate(zip(selected_train_series, selected_val_series_raw)):
        country_id = full_series_raw[selected_indices[idx]].components[0] # Get original country ID
        ...
        baseline_predictions_country_raw = baseline_country.predict(n=n_predict_baseline_country) # Get TimeSeries
        # Predictions are now directly in raw scale from HeuristicBaseline.predict()

        y_true_flat = val_ts_country_raw.values().flatten()
        y_pred_flat = baseline_predictions_country_raw.values().flatten() # Flatten directly for metrics
        ...
        train_ts_country.plot(ax=ax, label="Training Data (Log1p)", color='blue', alpha=0.7)
        
        # Plot validation data (raw scale)
        val_ts_country_raw.plot(ax=ax, label="Validation Data (Raw)", color='green', alpha=0.7)
        ...
        baseline_predictions_country_raw.plot(ax=ax, label="Baseline Prediction", color='red', linestyle='--', linewidth=2)
        
        # Add a vertical line for the split point
        split_date = train_ts_country.end_time()
        ax.axvline(split_date, color='grey', linestyle=':', alpha=0.8, label='Train/Validation Split')
    ...
    overall_msle = mean_squared_log_error(np.concatenate(all_plot_y_true), np.concatenate(all_plot_predictions_raw))
    overall_ratio = calculate_mean_pred_true_ratio(np.concatenate(all_plot_y_true), np.concatenate(all_plot_predictions_raw))
