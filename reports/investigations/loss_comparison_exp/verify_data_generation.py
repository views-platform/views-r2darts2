...
def main():
    ...
    os.makedirs(data_dir, exist_ok=True) # Ensure data_dir exists (created by data_generator but for robustness)
    ...
    for i, country_id in enumerate(random_countries):
        ...
        split_point_idx = int(len(country_df) * (1 - EXPERIMENT_CONFIG["test_split_ratio"]))
        split_date = country_df['time'].iloc[split_point_idx]
        ax.axvline(split_date, color='r', linestyle='--', alpha=0.7, label='Train/Validation Split')

        prop_zeros = timeline_stats_df[timeline_stats_df["country_id"] == country_id]["prop_zeros"].iloc[0]
