import pandas as pd
import numpy as np
from views_evaluation.evaluation.evaluation_manager import (
    EvaluationManager,
...
def generate_mock_data(
    ...
    num_prediction_sequences: int = 1,
    num_samples: int = 1,
):
    ...
    preds_1_index = pd.MultiIndex.from_product(
        [range(500, 500 + 3), [10, 20]], names=["month_id", location_id_name]
    )
    preds_1 = pd.DataFrame(
        # Note the required list format for point predictions
        {
            pred_col_name: [
                [val] * num_samples for val in np.random.rand(len(preds_1_index)) * 50
            ]
        },
        index=preds_1_index,
    )
    ...
    if num_prediction_sequences > 1:
        preds_2_index = pd.MultiIndex.from_product(
            [range(501, 501 + 3), [10, 20]], names=["month_id", location_id_name]
        )
        preds_2 = pd.DataFrame(
            {
                pred_col_name: [
                    [val] * num_samples
                    for val in np.random.rand(len(preds_2_index)) * 50
                ]
            },
            index=preds_2_index,
        )
...
# --- Test Harness ---
def run_evaluation_with_manager(
    actuals: pd.DataFrame,
    ...
    try:
        ...
        results = manager.evaluate(
            actual=actuals, predictions=predictions, target=target, config=eval_config
        )
...
def main():
    ...
    valid_actuals, valid_predictions = generate_mock_data(
        target_name, location_id_name, num_prediction_sequences=2
    )

    # --- Action C.2: Test Index Name Assumption (Malformed Index Level Name) ---
    malformed_index_actuals = valid_actuals.copy()
    malformed_index_actuals.index.names = [
        "month_id",
        "invalid_location_id",
    ]  # Violate index name
    run_evaluation_with_manager(
        malformed_index_actuals,
        valid_predictions,
        target_name,
        "Malformed Actuals Index Name",
    )
    ...
    malformed_col_preds[0].rename(
        columns={f"pred_{target_name}": "wrong_col_name"}, inplace=True
    )  # Violate column name
    run_evaluation_with_manager(
        valid_actuals,
        malformed_col_preds,
        target_name,
        "Malformed Prediction Column Name",
    )
    ...
    original_results = run_evaluation_with_manager(
        valid_actuals,
        valid_predictions,
        target_name,
        "Valid Predictions (Baseline for Order Check)",
    )
    shuffled_results = run_evaluation_with_manager(
        valid_actuals, shuffled_predictions, target_name, "Shuffled Predictions List"
    )
    ...
    if original_results and shuffled_results:
        ...
        if not np.isclose(
            original_rmsle, shuffled_rmsle
        ):  # Use np.isclose for float comparison
            print("  OBSERVATION: RMSLE changed due to shuffling. Order matters.")
        else:
            print(
                "  OBSERVATION: RMSLE did NOT change due to shuffling. Order might not matter for this metric or config."
            )
    ...
    run_evaluation_with_manager(
        valid_actuals, non_numeric_preds, target_name, "Non-Numeric Prediction Values"
    )
