...
def create_mock_dataset() -> _ViewsDatasetDarts:
    ...
    dataset = _ViewsDatasetDarts(
        source=df, targets=["lr_ged_sb_best"], broadcast_features=True
    )
...
def assert_contract_is_valid(df: pd.DataFrame, target: str, num_samples: int):
    ...
    assert isinstance(df.index, pd.MultiIndex), (
        f"FAIL: Index is {type(df.index)}, not MultiIndex."
    )
    print("PASS: Index is a MultiIndex.")
    assert len(df.index.levels) == 2, (
        f"FAIL: Index has {len(df.index.levels)} levels, not 2."
    )
    ...
    assert len(df.columns) == 1, (
        f"FAIL: DataFrame has {len(df.columns)} columns, not 1."
    )
    print("PASS: DataFrame has 1 column.")
    assert df.columns[0] == expected_col, (
        f"FAIL: Column name is '{df.columns[0]}', not '{expected_col}'."
    )
    ...
    assert isinstance(cell_value, list), (
        f"FAIL: Cell value is {type(cell_value)}, not list."
    )
    print("PASS: Cell value is a list.")
    assert len(cell_value) == num_samples, (
        f"FAIL: List contains {len(cell_value)} samples, not {num_samples}."
    )
    print(f"PASS: List contains {num_samples} sample(s).")
    assert all(isinstance(v, float) for v in cell_value), (
        "FAIL: Not all values in list are floats."
    )
...
def main():
    ...
    point_prediction_df = forecaster.predict(
        sequence_number=0, output_length=10, num_samples=1
    )
    ...
    prob_prediction_df = forecaster.predict(
        sequence_number=0, output_length=10, num_samples=5, mc_dropout=True
    )
