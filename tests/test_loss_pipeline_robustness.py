...
from views_r2darts2.utils.loss.time_aware_weighted_huber_loss import TimeAwareWeightedHuberLoss
from views_r2darts2.utils.loss.weighted_penalty_huber_loss import WeightedPenaltyHuberLoss
from views_r2darts2.utils.loss.zero_inflated_loss import ZeroInflatedLoss
...
def test_huber_delta_sensibility_across_pipelines(loss_class, pipeline):
    ...
    errors_np = (
        (np.random.rand(100) - 0.5) * (targets_np.max() - targets_np.min()) * 0.5
    )
...
def test_spikefocal_threshold_reachability_across_pipelines(pipeline):
    ...
    loss_functional = SpikeFocalLoss(
        alpha=0.8, gamma=2.0, spike_threshold=reachable_threshold
    )
