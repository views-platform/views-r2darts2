"""End-to-end parity test: parquet → FeatureFrame → Darts TimeSeries →
...
# Make the new package importable.
sys.path.insert(0, "/home/z/my-project/workspace/views-r2darts2-new")
...
PARQUET_PATH = "/home/z/my-project/upload/validation_viewser_df.parquet"
...
def test_parquet_bit_parity() -> None:
    ...
    for col in TARGETS + FEATURES:
        direct = pq.read_table(PARQUET_PATH, columns=[col]).column(col).to_numpy().astype(np.float32)
        idx = frame.feature_names.index(col)
        loaded = frame.values[:, idx, 0]
        assert direct.shape == loaded.shape, f"{col}: shape mismatch {direct.shape} vs {loaded.shape}"
        assert np.array_equal(direct, loaded), f"{col}: bit parity failed"
    print(f"  ✓ parquet_bit_parity: all {len(TARGETS) + len(FEATURES)} columns match")


def test_memmap_cache_parity() -> None:
    """Memmap-cached load must produce bit-identical values to in-memory load."""
    with tempfile.TemporaryDirectory() as cache_dir:
        # First read: decodes parquet, writes cache.
        ...
        assert np.array_equal(frame1.index.time, frame2.index.time), "time parity failed"
        assert np.array_equal(frame1.index.unit, frame2.index.unit), "unit parity failed"
    print(f"  ✓ memmap_cache_parity: values is memmap={isinstance(frame2.values, np.memmap)}, bit-identical")


def test_darts_bridge_parity() -> None:
    ...
    # Build per-entity TimeSeries (subset to first 3 entities for speed).
    ...
    # For each entity, verify the TimeSeries values match the frame values.
    ...
    for i, entity_id in enumerate(unique_entities):
        ts = series_list[i]
        # Filter the frame to this entity + sorted by time.
        ...
        assert ts_values.shape == expected_values.shape, (
            f"Entity {entity_id}: shape mismatch {ts_values.shape} vs {expected_values.shape}"
        )
        assert np.array_equal(ts_values.astype(np.float32), expected_values), (
            f"Entity {entity_id}: bit parity failed"
        )
    print(f"  ✓ darts_bridge_parity: {len(series_list)} entities, all values match")


def test_scaler_round_trip_parity() -> None:
    """AsinhTransform + MaxAbsScaler round-trip must recover values to float32 precision."""
    # Build a small synthetic test (no model needed).
    rng = np.random.default_rng(42)
    raw = rng.lognormal(mean=2.0, sigma=1.5, size=(1000, 3)).astype(np.float32)
    raw = np.maximum(raw, 0.0)

    # Use ScalerSelector to build the chain.
    ...
    # Fit + transform + inverse via the Darts Scaler (single-Scaler path).
    ...
    # Asinh/sinh round-trip should be near-perfect for non-negative inputs.
    # Allow tiny float32 epsilon.
    max_err = np.abs(inv_values.astype(np.float32) - raw).max()
    print(f"  ✓ scaler_round_trip_parity: max_err={max_err:.2e} (chain=AsinhTransform->MaxAbsScaler)")
...
def test_feature_scaler_manager_parity() -> None:
    ...
    # Build a small multi-entity dataset with the same column names as the user's mapping.
    n_time = 24
    n_entities = 5
    feature_cols = [
        "lr_ged_sb_delta", "lr_splag_1_ged_sb", "lr_decay_ged_sb_1",
        "lr_wdi_sm_pop_refg_or",  # different group
    ]
    ...
    # The user's mapping: AsinhTransform->MaxAbsScaler for the conflict columns.
    feature_scaler_map = {
    ...
    # Compare each entity: round-trip should be near-perfect.
    max_err = 0.0
    ...
    print(f"  ✓ feature_scaler_manager_parity: max_err={max_err:.2e} ({n_entities} entities, {len(feature_cols)} features)")
    assert max_err < 1e-3, f"Round-trip error too large: {max_err}"


def test_full_dataloader_flow() -> None:
    ...
    assert len(series_list) == n_entities_expected, (
        f"Expected {n_entities_expected} series, got {len(series_list)}"
    )

    # Verify total row count matches the frame.
    total_rows = sum(len(ts) for ts in series_list)
    assert total_rows == frame.n_rows, (
        f"Total rows across series ({total_rows}) != frame rows ({frame.n_rows})"
    )

    # Verify each series has the right column count.
    expected_cols = len(feats) + len(targs)
    for ts in series_list:
        assert len(ts.components) == expected_cols, (
            f"Series has {len(ts.components)} components, expected {expected_cols}"
        )
    print(f"  ✓ full_dataloader_flow: {n_entities_expected} entities, {total_rows} total rows, {expected_cols} cols/series")


def main() -> None:
    print("Running end-to-end parity tests against validation_viewser_df.parquet...")
    test_parquet_bit_parity()
    test_memmap_cache_parity()
    test_darts_bridge_parity()
    test_scaler_round_trip_parity()
    test_feature_scaler_manager_parity()
    test_full_dataloader_flow()
    print()
