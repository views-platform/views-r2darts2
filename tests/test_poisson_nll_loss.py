# file: tests/test_poisson_nll_loss.py
...
def test_poisson_nll_loss_with_darts_models(model_name, model_tuple, seed):
    """
    Tests that PoissonNLLLoss can be successfully used for training with various Darts models.
    ...
    try:
        ...
        pytest.fail(f"Integration test for {model_name} with PoissonNLLLoss and seed {seed} failed with error: {e}")
    ...
    try:
        ...
        pytest.fail(f"Prediction for {model_name} with PoissonNLLLoss and seed {seed} failed after training: {e}")
