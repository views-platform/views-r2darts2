...
class TestReproducibilityInfra:
    ...
    def test_partition_continuity_enforcement(self, mock_manager):
        ...
        from views_r2darts2.utils.gates import TemporalDiscontinuityError
        # Case 1: Gap detected
    ...
    def test_horizon_standard_warning(self, mock_manager):
        ...
        with patch('views_r2darts2.utils.gates.logger.warning') as mock_warn:
            ReproducibilityGate.Config.audit_architecture({"steps": list(range(1, 13)), "output_chunk_length": 1})
    ...
    def test_data_leakage_prevention(self, mock_manager):
        ...
        from views_r2darts2.utils.gates import DataLeakageError
        ...
        ts = TimeSeries.from_times_and_values(times, np.array([1.0, 2.0]), columns=["target"])
        
        with pytest.raises(DataLeakageError):
            ReproducibilityGate.Temporal.audit_boundary_integrity([ts], expected_end=150)
    ...
    def test_training_continuity_enforcement(self, mock_manager):
        ...
        from views_r2darts2.utils.gates import TemporalHoleError
        import numpy as np
    ...
    def test_stochastic_parity_serialization(self, tmp_path):
        ...
        if "Mock" in str(type(torch.load)):
            pytest.skip("Skipping Stochastic Parity test due to global mock-poisoning of torch.load")
    ...
    def test_starvation_kill_gate(self, mock_manager):
        ...
        """
        from views_r2darts2.utils.gates import DataStarvationError
        from darts import TimeSeries
        import pandas as pd
        ...
        with pytest.raises(DataStarvationError):
            ReproducibilityGate.Temporal.audit_boundary_integrity([ts], expected_end=100)
...
class TestRedTeamAttacks:
    ...
    @pytest.mark.parametrize("poison_key, poison_value", [
        ("random_state", None),
        ("lr", None),
        ("batch_size", None),
        ("steps", None),
    ...
    def test_off_by_one_peeking(self):
        ...
        ts = TimeSeries.from_dataframe(pd.DataFrame({"t":[1.0, 2.0],"time":[100,101]}), "time", "t", freq=1)
        with pytest.raises(DataLeakageError, match="beyond the allowed training boundary"):
            ReproducibilityGate.Temporal.audit_boundary_integrity([ts], expected_end=100)
    ...
    def test_off_by_one_starvation(self):
        ...
        ts = TimeSeries.from_dataframe(pd.DataFrame({"t":[1.0, 2.0],"time":[98,99]}), "time", "t", freq=1)
        with pytest.raises(DataStarvationError, match="throwing away your most recent history"):
            ReproducibilityGate.Temporal.audit_boundary_integrity([ts], expected_end=100)
    ...
    def test_fence_post_eval_overflow(self):
        ...
        from views_r2darts2.utils.gates import PredictionHorizonError
        ...
        with pytest.raises(PredictionHorizonError, match="PREDICTION OVERFLOW"):
            ReproducibilityGate.Temporal.audit_prediction_horizon(
                run_type="calibration", train_end=100, test_end=110, max_steps=10, total_sequences=2
            )
