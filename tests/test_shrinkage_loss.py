...
@pytest.mark.skip(
    reason="Plotting is a manual verification step, not for automated testing."
)
def test_plot_shrinkage_loss_landscape():
    ...
    plt.plot(
        errors.numpy(),
        manual_shrinkage_losses.numpy(),
        label=f"Shrinkage Loss (a={a}, c={c})",
        color="blue",
        linewidth=2,
    )
    plt.plot(
...
def test_shrinkage_loss_with_darts_models(model_name, model_tuple, seed):
    ...
    try:
        ...
        pytest.fail(
            f"Integration test for {model_name} with ShrinkageLoss and seed {seed} failed with error: {e}"
        )
    ...
    try:
        ...
        pytest.fail(
            f"Prediction for {model_name} with ShrinkageLoss and seed {seed} failed after training: {e}"
        )
...
def test_shrinkage_loss_gradient_check():
    ...
    targets_near_c = torch.tensor(
        [[0.1], [0.8]], dtype=torch.double
    )  # errors are 0.2 and 0.3
