...
def test_zero_inflated_loss_golden_values(pred_val, target_val, expected_loss, loss_params):
...
def test_zero_inflated_loss_invariants(loss_params):
    ...
    expected_zero_loss = torch.nn.functional.binary_cross_entropy(p_zero, is_zero, reduction='mean')
    ...
    expected_count_loss = torch.mean(delta**2 * (torch.sqrt(1 + (errors / delta)**2) - 1))
