...
class DartsForecastingModelManager(_PARENT_CLASS):  # type: ignore[misc, valid-type]
    ...
    def _infer_cache_source_label(self) -> str:
        """Infer the cache source label used in raw dataloader filenames."""
        ...
        if isinstance(queryset, dict):
            ...
            if source == "viewser":
                return "viewser"
        raise ValueError(f"Unrecognized source label in queryset: {queryset}")

    def _resolve_raw_parquet_path(self, run_type: str) -> Path:
        """Resolve the raw parquet path across source-aware cache spellings."""
