...
def apply_rinorm_compression_patch():
    ...
    def _raw_space_inverse(self, x: torch.Tensor):
        ...
        # Cap per-series sigma relative to the batch-mean sigma softly
        # to prevent RevIN denorm from amplifying extreme-conflict series
        # into forecast runaway, without using hard clamps.
        # sigma_batch_mean = sigma.mean(dim=0, keepdim=True)
        # We cap sigma at some multiple without a hard threshold using a softplus or tanh
        ...
        # Expand: sinh(ẑ) · σ_safe gives centered-raw prediction
        x = torch.asinh(torch.sinh(x) * sigma_safe) + mu
