...
class SpotlightLossLogcosh(torch.nn.Module):
    ...
    ── Components ───────────────────────────────────────────────────────
     ...
     1. **DC/AC decomposition** — full-series demeaning.
         e_shape = e − mean_t(e).  Shape and level are orthogonal:
         shape handles within-series patterns, level handles pointwise DC.
    ...
    3. **Per-series temporal DRO (event-gated)** — within-series shock therapy.
     ...
     4. **Per-cell level anchor** — log_cosh on raw asinh error, weighted by
         the same event mass used for shape and self-normalized over the batch.
    ...
    def _level_loss_per_cell(
        ...
        num = weighted.sum(dim=(0, 1))
        den = event_mag.sum(dim=(0, 1)).clamp_min(self._EMA_EPS)
        return num / den
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        # ── Full-series DC/AC decomposition ───────────────────────────
        # Demean over the whole forecast horizon so shape is series-level,
        # not windowed. This keeps temporal pattern learning while removing
        # the window mechanism entirely.
