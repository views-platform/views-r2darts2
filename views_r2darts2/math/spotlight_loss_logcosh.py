...
class SpotlightLossLogcosh(torch.nn.Module):
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        if multivariate:
            loss_level = T * math.sqrt(T) * (w_level * level_cell).sum(dim=0) / w_level.sum(dim=0).clamp_min(self._EPS)
        else:
            loss_level = T * math.sqrt(T) * (w_level * level_cell).sum() / w_level.sum().clamp_min(self._EPS)
