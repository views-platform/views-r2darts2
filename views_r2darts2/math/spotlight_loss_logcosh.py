...
class SpotlightLossLogcosh(torch.nn.Module):
    ...
    def _windowed_level_loss(
        ...
        # ── Sum-over-windows level anchor + Hájek over series ─────────
        # First aggregate per-series window losses by SUM over windows,
        # then apply composition-robust Hájek normalization across series:
        #   L = scale * Σ(series_w * Σ_j level_{b,j}) / Σ(series_w)
        #
        # This preserves the requested sum-over-windows behavior while keeping
        # cross-series scale invariant to event composition.
        #
        # scale_factor = W compensates the 1/W gradient attenuation introduced
        # by the window mean operator, yielding a stable per-cell DC signal.
        scale_factor = W
        if level_losses.dim() == 3:
            per_series = level_losses.sum(dim=1)  # (B, C)
            num = (series_w * per_series).sum(dim=0)                    # (C,)
            den = series_w.sum(dim=0).clamp(min=self._EMA_EPS)          # (C,)
            return scale_factor * num / den                             # (C,)
        else:
            per_series = level_losses.sum(dim=1)  # (B,)
            num = (series_w * per_series).sum()
