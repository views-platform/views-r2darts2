...
class SpotlightLossLogcosh(torch.nn.Module):
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        n_eff = gate.sum(dim=1).clamp_min(1.0).mean().item()
        level_cell = self._log_cosh(gap)
        w_level = gate.amax(dim=1)

        if multivariate:
            loss_level = (T + n_eff) * (w_level * level_cell).sum(dim=0) / w_level.sum(dim=0).clamp_min(self._EPS)
        else:
            loss_level = (T + n_eff) * (w_level * level_cell).sum() / w_level.sum().clamp_min(self._EPS)
