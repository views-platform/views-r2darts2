...
class SpotlightLossLogcosh(torch.nn.Module):
    ...
    def __init__(self, non_zero_threshold: float = 0.88):
        ...
        logger.info(
            "SpotlightLossV61 | threshold=%.4f K=%d | event-level T-scaled + zero-anchor",
            non_zero_threshold,
            self._K,
        )
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        if multivariate:
            loss_shape = (
                (shape_w * shape_cell).sum(dim=(0, 1))
                / shape_w.sum(dim=(0, 1)).clamp_min(self._EPS)
            )
        else:
            loss_shape = (
                (shape_w * shape_cell).sum()
                / shape_w.sum().clamp_min(self._EPS)
            )

        # ── LEVEL: event-conditioned, asinh_plus, horizon-scaled ─────
        # This is the DC spring.  It must be strong.
        ...
        # ── ZERO-ANCHOR: per-series normalization, no T ──────────────
        # Pushes non-event cells toward zero in asinh space.
        # Do NOT multiply by T here; that would overpower the level term.
        zero_w = 1.0 - gate
        zero_cell = self._log_cosh(y_pred)

        if multivariate:
            loss_zero = (zero_w * zero_cell).sum(dim=(0, 1)) / B
        else:
            loss_zero = (zero_w * zero_cell).sum() / B
        ...
        with torch.no_grad():
            # Signed gap and level gradient proxy are very useful here.
            level_grad = torch.asinh(gap) + gap / torch.sqrt(1.0 + gap.pow(2))

            if multivariate:
                gap_signed_l = gap.mean(dim=0).tolist()
                level_grad_l = level_grad.mean(dim=0).tolist()
                ...
                gap_signed_l = [gap.mean().item()]
                level_grad_l = [level_grad.mean().item()]
        ...
        if torch.isnan(total_loss):
            raise RuntimeError(f"NaN in SpotlightLossV61: per_channel={comp}")
        ...
        self._last_components = {
            ...
            "level_gap_signed": gap_signed_l,
            "level_grad": level_grad_l,
        ...
        logger.debug(
            "SpotlightLossV61 | shape=%s level=%s zero=%s total=%.6f",
            shape_c,
            level_c,
            zero_c,
            total_loss.item(),
    ...
    def __repr__(self) -> str:
        return f"SpotlightLossV61(non_zero_threshold={self.tau})"